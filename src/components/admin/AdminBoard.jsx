...
const boardColumns = [
  {
    ...
    cards: [
      {
        title: "Verifikasi berkas pendaftar Paket B",
        tags: ["Pendaftaran", "Prioritas"],
        tint: "bg-blue-100 text-blue-900",
        progress: 40,
        people: ["RA", "DS"],
        due: "12",
        files: "4",
      ...
      {
        title: "Susun jadwal pembelajaran minggu depan",
        tags: ["Jadwal", "Pengajar"],
      ...
      {
        title: "Perbarui data warga belajar aktif",
        tags: ["Data WB"],
        tint: "bg-rose-100 text-rose-900",
        progress: 60,
        people: ["AS", "RA"],
        due: "18",
        files: "5",
  ...
  },
  {
    ...
    cards: [
      {
        title: "Rekap nilai evaluasi akhir semester",
        tags: ["Akademik", "Nilai"],
      ...
      {
        title: "Siapkan materi pelatihan keterampilan",
        tags: ["Program", "Modul"],
  ...
  },
  {
    ...
    cards: [
      {
        title: "Publikasi kegiatan Hari Kemerdekaan",
        tags: ["Berita", "Publikasi"],
        tint: "bg-pink-100 text-pink-900",
        progress: 100,
        people: ["RA", "DS"],
        due: "03",
        files: "6",
      ...
      {
        title: "Persetujuan laporan kegiatan Juli",
        tags: ["Laporan"],
        tint: "bg-indigo-100 text-indigo-900",
        progress: 90,
        people: ["AS"],
        due: "09",
        files: "2",
      ...
      {
        title: "Review usulan program warga belajar",
        tags: ["Program", "Review"],
...
const TaskCard = ({ card, marker }) => (
  <article
    className={`${card.tint} rounded-2xl p-4 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md`}
  >
    <div className="mb-4 flex items-start justify-between gap-2">
      <div className="flex flex-wrap gap-1.5">
        {card.tags.map((tag) => (
          <span
            key={tag}
            className="rounded-md bg-white/65 px-2 py-1 text-[10px] font-semibold"
          >
            #{tag}
          </span>
        ))}
      ...
      <button
        type="button"
        aria-label={`Opsi ${card.title}`}
        className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-white/45 text-lg leading-none"
      >
        ⋮
      </button>
    </div>
    ...
    <section className="mx-auto w-full max-w-[1440px]">
        ...
        <div className="flex flex-1 items-center justify-between gap-3">
          <div>
            <p className="text-lg font-bold text-slate-900">
              Board <span className="text-slate-400">–</span> Aktivitas PKBM
            </p>
            <p className="text-xs text-slate-500">
              Pantau pekerjaan tim dalam satu tempat
            </p>
          </div>
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        {boardColumns.map((column) => (
          <section
            key={column.title}
            className="rounded-3xl border border-slate-200 bg-white/70 p-3 shadow-sm"
          >
            <div className="mb-3 flex items-center justify-between border-b border-slate-200 px-2 pb-3">
              <h2 className="flex items-center gap-2 text-sm font-bold text-slate-800">
                <span className={`h-2.5 w-2.5 rounded-full ${column.marker}`} />
                {column.title}
              </h2>
              <div className="flex gap-3 text-lg text-slate-500">
                <button type="button" aria-label={`Tambah ke ${column.title}`}>
                  +
                </button>
                <button type="button" aria-label={`Menu ${column.title}`}>
                  ⋮
                </button>
              </div>
            </div>
            <div className="space-y-3">
              {column.cards.map((card) => (
                <TaskCard key={card.title} card={card} marker={column.marker} />
              ))}
            </div>
          </section>
        ))}
