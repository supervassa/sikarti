...
const AdminLayout = () => {
    ...
    const adminNavItems = [
        ...
        { path: '/admin/pendaftar', label: 'Pendaftar Calon WB', icon: faClipboardList },
        { path: '/admin/konten', label: 'Berita & Pengumuman', icon: faNewspaper },
        { path: '/admin/presensi', label: 'Rekap Kehadiran', icon: faCalendarCheck },
        { path: '/admin/keuangan', label: 'Informasi Tagihan', icon: faCreditCard },
    ];

    // Menu Khusus Superadmin (Hanya muncul untuk Superadmin)
    const superadminNavItems = [
        { path: '/superadmin/dashboard', label: 'Superadmin Dashboard', icon: faShieldHalved },
        { path: '/superadmin/admins', label: 'Manajemen Admin', icon: faUsersGear },
    ];

    const isActive = (path) => location.pathname === path;
    const displayName = currentUser?.nama || currentUser?.displayName || 'Pengguna';
                        ...
                        <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
                            ...
                            {!collapsed && (
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 capitalize">Portal {role}</p>
                                </div>
                            )}
                        </div>
                        <button
                    ...
                    {role === 'superadmin' && (
                        <div className="mb-6">
                            ...
                            <nav className="space-y-1">
                                {superadminNavItems.map((item) => renderNavItem(item, 'violet'))}
                            </nav>
                        </div>
                    )}
