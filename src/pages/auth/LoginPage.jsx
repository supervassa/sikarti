...
const readInitialMode = () => {
  try {
    return localStorage.getItem("loginMode") === "staf" ? "staf" : "wb";
  } catch {
    return "wb";
  }
};
...
const LoginPage = () => {
  // "wb" = login Warga Belajar pakai Nomor Induk; "staf" = Pengajar & Staf pakai email.
  ...
  const switchMode = (next) => {
    ...
    setMode(next);
    try {
      localStorage.setItem("loginMode", next);
    } catch {
      // localStorage tidak tersedia — abaikan, mode tetap jalan untuk sesi ini.
    }
    setErrorMsg("");
    setPassword("");
  ...
  const handleWbLogin = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    setErrorMsg("");
    clearAuthError();
    try {
      await loginWithNomorInduk(nomorInduk.trim(), password);
      // Redirect otomatis diurus oleh useEffect di atas jika berhasil
    } catch {
      setErrorMsg("Nomor Induk atau Password salah. Coba lagi.");
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div className="bg-[#0b1f3d] py-6 px-8 flex flex-col items-center">
          ...
          <p className="text-sm text-gray-300 mt-1 text-center">
            {mode === "wb"
              ? "Untuk Warga Belajar PKBM KARTINI."
              : "Untuk Pengajar dan Staf PKBM KARTINI."}
          </p>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
            <button
              ...
              className={tabClass(mode === "wb")}
            >
              Warga Belajar
            </button>
            <button
              type="button"
              onClick={() => switchMode("staf")}
            ...
            <>
              <form onSubmit={handleWbLogin} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={busy}
                  className={`w-full py-3 px-4 bg-red-600 text-white text-lg font-bold rounded-lg shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all ${busy ? "opacity-70 cursor-not-allowed" : ""}`}
                >
