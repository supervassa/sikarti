...
import { faPlay, faStop, faCircleCheck, faUsers } from '@fortawesome/free-solid-svg-icons';
import { db } from '../../config/firebase';
import { useAuth } from '../../context/AuthContext';
import { startClassSession, stopClassSession, finalizeAbsentees } from '../../services/pengajarServices';
...
const JadwalPengajar = () => {
    ...
    useEffect(() => {
        const q = query(collection(db, 'classSessions'), where('tanggal', '==', todayISO()));
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setSessionsToday(snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
        });
        return unsubscribe;
    }, []);
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(query(collection(db, 'users'), where('kd_role', '==', 22)), (snapshot) => {
    ...
    useEffect(() => {
        ...
        if (!session) return;
        const q = query(collection(db, 'presensi'), where('sessionId', '==', session.id));
        const unsubscribe = onSnapshot(q, (snapshot) => {
    ...
    useEffect(() => {
        sessionsToday.forEach((session) => {
            ...
            if (!startedMs) return;
            const ended = !!session.stoppedAt || now >= startedMs + session.durasiMenit * 60000;
            if (!ended) return;

            finalizeAttempted.current.add(session.id);
            const roster = wbList.filter((wb) => wb.paket === session.paket && normalizeWBStatus(wb.status) === 'AKTIF');
    ...
    const jadwalSaya = useMemo(() => jadwal.filter((j) => sameName(j.pengajar, currentUser?.nama)), [jadwal, currentUser?.nama]);
    const grouped = useMemo(() => HARI_ORDER
        .map((hari) => ({
            hari,
            items: jadwalSaya.filter((j) => j.hari === hari).sort((a, b) => a.jamMulai.localeCompare(b.jamMulai)),
        }))
        .filter((g) => g.items.length > 0), [jadwalSaya]);

    const hariIni = HARI_INDEX[new Date().getDay()];
    const sessionFor = (scheduleId) => sessionsToday.find((s) => s.scheduleId === scheduleId);
    const rosterFor = (item) => wbList.filter((wb) => wb.paket === item.paket && normalizeWBStatus(wb.status) === 'AKTIF');
    ...
    const handleStart = async ({ fotoBase64, lokasi }) => {
        setStarting(true);
        try {
            await startClassSession(activeSchedule, todayISO(), currentUser, { fotoBase64, lokasi });
    ...
    const handleStop = async (session) => {
        if (!confirm(`Hentikan sesi "${session.namaMapel}" sekarang? Kehadiran akan langsung dicatat.`)) return;
        setStopping(true);
        try {
    ...
    const renderAction = (item) => {
        ...
        if (!session) {
            if (item.hari !== hariIni) {
                return <p className="mt-2 text-xs text-slate-400">Jadwal untuk hari {item.hari} — belum bisa dimulai hari ini.</p>;
            }
            return (
                <button
                    type="button"
        ...
        return (
            <div className="mt-2 space-y-2">
                <div className="px-3 py-2 bg-emerald-50 dark:bg-emerald-500/10 rounded-lg text-center">
                    <p className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wide">Sesi Berlangsung</p>
                    <p className="text-lg font-bold text-emerald-800 dark:text-emerald-300 tabular-nums">{formatCountdown(remaining)}</p>
                </div>
                <button
                    type="button"
                    disabled={stopping}
        ...
        <section className="space-y-6">
            <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                    Tap <span className="font-semibold">Mulai Mengajar</span> pada jadwal hari ini untuk membuka presensi WB.
                </p>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {grouped.map((g) => (
                        <div key={g.hari} className={`bg-white dark:bg-slate-900 rounded-2xl border shadow-sm overflow-hidden ${g.hari === hariIni ? 'border-red-300 dark:border-red-500/40' : 'border-slate-200 dark:border-slate-800'}`}>
                            <div className={`px-4 py-2.5 font-semibold text-sm text-white ${g.hari === hariIni ? 'bg-red-600' : 'bg-slate-700'}`}>
                                {g.hari}{g.hari === hariIni ? ' · Hari Ini' : ''}
                            </div>
            ...
            {detailItem && (
                <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-xl max-h-[85vh] overflow-y-auto">
                        <div>
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white">{detailItem.namaMapel}</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                {detailItem.paket} · {detailItem.hari} {detailItem.jamMulai}-{detailItem.jamSelesai}
                            </p>
                        </div>

                        <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
                            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-emerald-500" /> Sudah presensi</span>
                            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-slate-300 dark:bg-slate-600" /> Belum presensi</span>
                        </div>

                        {!sessionFor(detailItem.id) && (
                            <p className="text-xs text-amber-600">Sesi belum dimulai hari ini — semua WB masih berstatus belum presensi.</p>
                        )}

                        <div className="divide-y divide-slate-100 dark:divide-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden">
                            {rosterFor(detailItem).length === 0 ? (
                                <p className="px-4 py-6 text-center text-sm text-slate-400">Belum ada WB aktif di paket ini.</p>
                            ) : (
                                rosterFor(detailItem).map((wb) => {
                                    ...
                                    return (
                                        <div key={wb.id} className="flex items-center gap-3 px-4 py-2.5">
                                            <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${isCheckedIn ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`} />
                                            <span className="text-sm text-slate-700 dark:text-slate-200">{wb.nama}</span>
                                        </div>
                                    );
                                })
                            )}
                        ...
                        <div className="flex justify-end pt-1">
                            <button type="button" onClick={() => setDetailItem(null)} className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300 font-semibold">
                                Tutup
                            </button>
                        </div>
                    </div>
