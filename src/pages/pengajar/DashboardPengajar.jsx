...
const DashboardPengajar = () => {
  ...
  const jadwalSaya = jadwal.filter((j) =>
    sameName(j.pengajar, currentUser?.nama),
  );
  const hariIni = HARI_INDEX[new Date().getDay()];
  const jadwalHariIni = jadwalSaya
    .filter((j) => j.hari === hariIni)
    .sort((a, b) => a.jamMulai.localeCompare(b.jamMulai));

  const cards = [
    {
      label: "Total Jadwal Mengajar",
      value: jadwalSaya.length,
      icon: faCalendarDays,
    ...
    {
      label: "Status Akun",
      value: currentUser?.status !== false ? "Aktif" : "Nonaktif",
      icon: faCircleCheck,
      accent:
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {j.paket} · {j.jamMulai} - {j.jamSelesai}
                  </p>
                </div>
              </div>
            ))
