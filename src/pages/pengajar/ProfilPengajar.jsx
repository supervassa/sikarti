    ...
    return (
        <section className="space-y-6 max-w-xl">
            <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Data diri Anda di PKBM KARTINI.</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p className="font-semibold text-slate-800 dark:text-slate-100 mt-0.5">{currentUser?.nama}</p>
                    </div>
                    <div>
                        <p className="text-xs text-slate-400">Email</p>
                        <p className="font-semibold text-slate-800 dark:text-slate-100 mt-0.5">{currentUser?.email}</p>
                    </div>
                    <div>
                        <p className="text-xs text-slate-400">Mata Pelajaran Diampu</p>
                        <p className="font-semibold text-slate-800 dark:text-slate-100 mt-0.5">{currentUser?.mapelDiampu || '-'}</p>
                    </div>
                    <div>
                        <p className="text-xs text-slate-400">Status Akun</p>
                        <p className="font-semibold text-slate-800 dark:text-slate-100 mt-0.5">{currentUser?.status !== false ? 'Aktif' : 'Nonaktif'}</p>
                    </div>
                </div>

                <form onSubmit={handleSave} className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
                    <div>
                        <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Nomor HP</label>
                        <input
                            type="tel"
                            value={noHp}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 flex items-center justify-between gap-4">
                <div>
                    <p className="font-semibold text-slate-800 dark:text-slate-100">Ganti Password</p>
