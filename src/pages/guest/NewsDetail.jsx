...
const NewsDetail = () => {
  ...
  const videoEmbed =
    news?.tipeMedia === "video" ? getVideoEmbed(news.mediaUrl) : null;
            ...
            <>
              ...
              <header className="mb-10 text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400">
                  Dipublikasikan pada {formatNewsDate(news.createdAt)} &middot;{" "}
                  {news.author}
                </p>
              </header>
              ...
              {news.tipeMedia === "video" ? (
                videoEmbed.kind === "iframe" ? (
                  <div
                    className={`${
                      videoEmbed.portrait
                        ? "w-full max-w-[360px] mx-auto aspect-[9/16]"
                        : "w-full aspect-video"
                    } bg-black rounded-2xl mb-10 overflow-hidden`}
                  >
                    <iframe
                      src={videoEmbed.url}
                      title={news.title}
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-4 mb-10 rounded-2xl border border-gray-200 dark:border-slate-700 text-red-600 dark:text-red-400 font-semibold hover:bg-red-50 dark:hover:bg-red-500/10"
                  >
                    Tonton video di sumbernya &#8599;
                  </a>
                )
              ) : imageSrc ? (
                <img
                  src={imageSrc}
                  alt={news.title}
                  className="w-full max-h-[480px] object-cover rounded-2xl mb-10"
                />
              ) : null}
