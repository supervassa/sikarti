...
const SessionStatus = ({ session, now }) => {
  ...
  if (session.stoppedAt) {
    return (
      <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400">
        Dihentikan
      </span>
    );
  }
  ...
  const stillActive =
    startedMs && now < startedMs + session.durasiMenit * 60000;
  return stillActive ? (
...
const PresensiPage = () => {
  ...
  useEffect(() => {
    ...
    const stopPresensi = onSnapshot(
      query(collection(db, "presensi"), orderBy("createdAt", "desc")),
      (snap) => {
        setPresensi(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
        setLoading(false);
      },
    );
    const stopSesi = onSnapshot(
      query(collection(db, "classSessions"), orderBy("startedAt", "desc")),
      (snap) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Catatan kehadiran Warga Belajar dan Pengajar.
          </p>
        </div>
        {tab === "wb" && (
          <button
                  ...
                  presensi.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-bold ${STATUS_STYLE[p.status] || "bg-slate-100 text-slate-600"}`}
                        >
                          {p.status}
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 -mt-2">
              Untuk mencatat Izin/Sakit/Alpa. WB hadir tercatat otomatis lewat
              presensi mandiri.
            </p>
