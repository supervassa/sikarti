...
import { createBerita } from '../../services/adminServices';
...
const BeritaManagementPage = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setListBerita(snapshot.docs.map((item) => ({ id: item.id, ...item.data() })));
            setLoading(false);
        });
        return unsubscribe;
    }, []);
    ...
    const filtered = useMemo(() => {
        ...
        return listBerita.filter((item) => item.title?.toLowerCase().includes(keyword));
    }, [listBerita, search]);
    ...
    const handleFileChange = async (e) => {
        ...
        if (file.size > MAX_UPLOAD_BYTES) {
            alert(`Ukuran gambar maksimal ${Math.round(MAX_UPLOAD_BYTES / 1024)}KB. Kompres gambar atau gunakan URL.`);
            e.target.value = '';
            return;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                mediaUrl: form.tipeMedia === 'video' || form.mediaType === 'url' ? form.mediaUrl.trim() : undefined,
                mediaBase64: form.tipeMedia === 'gambar' && form.mediaType === 'upload' ? form.mediaBase64 : undefined,
            };
            await createBerita(payload, currentUser);
            resetForm();
            setIsModalOpen(false);
        } catch (err) {
            alert('Gagal menambah berita: ' + err.message);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <section className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Berita & Pengumuman</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        Kelola berita dan pengumuman yang tampil di halaman publik PKBM KARTINI.
                    </p>
                </div>
                <button
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 w-full sm:max-w-xs">
                    <FontAwesomeIcon icon={faMagnifyingGlass} className="w-3.5 h-3.5 text-slate-400" />
                    <input
                        type="text"
                        value={search}
                        onChange={(e) => {
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        {loading ? (
                            <tr>
                                <td colSpan="4" className="text-center py-8 text-slate-400">Memuat data berita...</td>
                            </tr>
                        ) : paginated.length === 0 ? (
                            <tr>
                                <td colSpan="4" className="text-center py-10 text-slate-400">
                                    <FontAwesomeIcon icon={faNewspaper} className="w-6 h-6 mb-2 opacity-50" />
                                ...
                                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                                    ...
                                    <td className="px-4 py-4">
                                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20">
                                            <FontAwesomeIcon icon={item.tipeMedia === 'video' ? faVideo : faImage} className="w-3 h-3" />
                                            {item.tipeMedia === 'video' ? 'Video' : 'Gambar'}
                                        </span>
                                    </td>
                                    <td className="px-4 py-4 text-slate-600 dark:text-slate-300">{item.author || '-'}</td>
                                    <td className="px-4 py-4 text-xs text-slate-400">{formatTanggal(item.createdAt)}</td>
                                </tr>
                            ))
                        )}
                        </tbody>
                ...
                {!loading && filtered.length > 0 && (
                    <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 dark:border-slate-700 text-sm text-slate-500 dark:text-slate-400">
                        <span>
                            Halaman {currentPage} dari {totalPages} &middot; {filtered.length} berita
                        </span>
                ...
                <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4 overflow-y-auto">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-xl my-8">
                        <div className="flex items-center justify-between">
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Tambah Berita</h2>
                            <button
                                type="button"
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Judul</label>
                                <input
                                    required
                                    type="text"
                                    value={form.title}
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Deskripsi</label>
                                <textarea
                                    required
                                    rows={4}
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-2">Tipe Media</label>
                                    ...
                                    {[
                                        ...
                                        >
                                            <input
                                                ...
                                                onChange={() => setForm({ ...form, tipeMedia: opt.value, mediaType: 'upload', mediaUrl: '', mediaBase64: '', fileName: '' })}
                                                className="hidden"
                                            />
                            ...
                            {form.tipeMedia === 'gambar' ? (
                                <div className="space-y-3">
                                        ...
                                        {[
                                            ...
                                            >
                                                <input
                                                    ...
                                                    onChange={() => setForm({ ...form, mediaType: opt.value, mediaUrl: '', mediaBase64: '', fileName: '' })}
                                                    className="hidden"
                                                />
                                    ...
                                    {form.mediaType === 'upload' ? (
                                        <div>
                                            ...
                                            />
                                            {form.fileName && (
                                                <p className="text-xs text-slate-400 mt-1">Terpilih: {form.fileName}</p>
                                            )}
                                            <p className="text-[11px] text-slate-400 mt-1">Maks {Math.round(MAX_UPLOAD_BYTES / 1024)}KB.</p>
                                        </div>
                                    ) : (
                                        <input
                                            required
                                ...
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">URL Video</label>
                                    <input
                                        required
                                        type="url"
