...
const DashboardOverviewPage = () => {
  ...
  useEffect(() => {
    const stopWB = onSnapshot(
      ...
      (snap) => setTotalWB(snap.size),
    );
    const stopPengajar = onSnapshot(
      query(collection(db, "users"), where("kd_role", "==", 33)),
      (snap) => setTotalPengajar(snap.size),
    );
    const stopPendaftar = onSnapshot(
      query(collection(db, "registrations"), where("status", "==", "PENDING")),
      (snap) => setPendaftarPending(snap.size),
    );
    const stopBerita = onSnapshot(collection(db, "news"), (snap) =>
      setTotalBerita(snap.size),
    );
    // Firestore rules batasi baca auditLogs khusus superadmin.
    const stopLogs =
      role === "superadmin"
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Ringkasan operasional PKBM KARTINI hari ini.
        </p>
      </div>
