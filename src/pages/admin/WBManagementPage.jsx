...
import { createWB, updateWB, normalizeWBStatus, setWBLifecycleStatus, deleteWB } from '../../services/adminServices';
...
const WBManagementPage = () => {
    ...
    const [editForm, setEditForm] = useState({ nama: '', email: '', paket: 'Paket C', tahunAngkatan: DEFAULT_TAHUN_ANGKATAN, nik: '', noHp: '' });
    const [statusForm, setStatusForm] = useState({ status: 'AKTIF', tahunLulus: DEFAULT_TAHUN_ANGKATAN, terakhirAktif: DEFAULT_TAHUN_ANGKATAN });
    ...
    const filteredWB = useMemo(() => {
        return listWB.filter((wb) => {
            if (searchNama && !wb.nama?.toLowerCase().includes(searchNama.toLowerCase())) return false;
            if (filterPaket !== 'Semua' && (wb.paket || 'Paket C') !== filterPaket) return false;
            if (filterTahun !== 'Semua' && wb.tahunAngkatan !== filterTahun) return false;
            return true;
        });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setForm({ nama: '', email: '', paket: 'Paket C', tahunAngkatan: DEFAULT_TAHUN_ANGKATAN, nik: '', noHp: '' });
            setIsModalOpen(false);
            alert('Warga Belajar berhasil ditambahkan! Email set password telah dikirim.');
        } catch (err) {
            alert(err.code === 'auth/email-already-in-use' ? 'Email tersebut sudah digunakan.' : 'Gagal menambah data WB: ' + err.message);
        }
    };
    ...
    const handleStatusSave = async () => {
        ...
        try {
            await setWBLifecycleStatus({ id: detailWB.id, ...statusForm }, currentUser);
    ...
    const handleDelete = async () => {
        if (!confirm(`Hapus data ${detailWB.nama} dari sistem? Akses WB ini ke aplikasi akan langsung dicabut.`)) return;
        setSaving(true);
        try {
    ...
    const renderStatusBadge = (wb) => {
        ...
        return (
            <div>
                ...
                {status === 'LULUS' && <p className="text-[10px] text-slate-400 mt-1">Lulus {wb.tahunLulus}</p>}
                {status === 'NONAKTIF' && <p className="text-[10px] text-slate-400 mt-1">Terakhir aktif {wb.terakhirAktif}</p>}
            </div>
        );
    };
        ...
        <section className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-col sm:flex-row gap-3">
                <div className="flex-1 flex items-center gap-2 bg-slate-100 rounded-xl px-3 py-2.5">
                    <FontAwesomeIcon icon={faMagnifyingGlass} className="text-slate-400 w-3.5 h-3.5" />
                    <input
                        type="text"
                ...
                >
                    ...
                    {tahunOptionsFromData.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:border-red-500"
                                    >
                                        {TAHUN_ANGKATAN_OPTIONS.map((t) => <option key={t} value={t}>{t}</option>)}
                                    </select>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between">
                            <h2 className="text-lg font-bold text-slate-900">Detail Warga Belajar</h2>
                            <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${STATUS_META[normalizeWBStatus(detailWB.status)].className}`}>
                        ...
                        <form onSubmit={handleUpdate} className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-[11px] text-amber-600">
                                    Ini hanya mengubah email kontak di sistem. Email login (Firebase Auth) tetap yang lama sampai WB reset password sendiri.
                                </p>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setEditForm({ ...editForm, tahunAngkatan: e.target.value })}
                                        className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:border-red-500"
                                    >
                                        {tahunOptionsFromData.map((t) => <option key={t} value={t}>{t}</option>)}
                                    </select>
                        ...
                        <div className="pt-3 border-t border-slate-100 space-y-3">
                            <label className="block text-xs font-semibold text-slate-600">Status Keanggotaan</label>
                            <select
                            ...
                            {statusForm.status === 'LULUS' && (
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">Tahun Lulus</label>
                                    <select
                                        value={statusForm.tahunLulus}
                                        onChange={(e) => setStatusForm({ ...statusForm, tahunLulus: e.target.value })}
                                        className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:border-red-500"
                                    >
                                        {tahunOptionsFromData.map((t) => <option key={t} value={t}>{t}</option>)}
                                    </select>
                            ...
                            {statusForm.status === 'NONAKTIF' && (
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1">Terakhir Aktif</label>
                                    <select
                                        value={statusForm.terakhirAktif}
                                        onChange={(e) => setStatusForm({ ...statusForm, terakhirAktif: e.target.value })}
                                        className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:border-red-500"
                                    >
                                        {tahunOptionsFromData.map((t) => <option key={t} value={t}>{t}</option>)}
                                    </select>
