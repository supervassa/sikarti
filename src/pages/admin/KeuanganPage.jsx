...
import { createTagihan, updateTagihanStatus } from '../../services/adminServices';
...
const KeuanganPage = () => {
    ...
    const [form, setForm] = useState({ wbId: '', nama: '', keterangan: 'SPP Bulanan', jumlah: '', jatuhTempo: '' });

    useEffect(() => {
        const stopWB = onSnapshot(query(collection(db, 'users'), where('kd_role', '==', 22)), (snap) => {
            setWbList(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
        });
        const stopTagihan = onSnapshot(query(collection(db, 'invoices'), orderBy('createdAt', 'desc')), (snap) => {
            setTagihan(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
            setLoading(false);
        });
        return () => { stopWB(); stopTagihan(); };
    }, []);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await createTagihan({ ...form, jumlah: Number(form.jumlah) }, currentUser);
            setForm({ wbId: '', nama: '', keterangan: 'SPP Bulanan', jumlah: '', jatuhTempo: '' });
    ...
    const toggleLunas = async (item) => {
        ...
        try {
            await updateTagihanStatus(item.id, item.status === 'LUNAS' ? 'BELUM_LUNAS' : 'LUNAS', currentUser);
        } catch (err) {
        ...
        <section className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">Kelola tagihan SPP dan pembayaran Warga Belajar.</p>
                </div>
                <button
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        {loading ? (
                            <tr><td colSpan="6" className="text-center py-8 text-slate-400">Memuat data tagihan...</td></tr>
                        ) : tagihan.length === 0 ? (
                            <tr><td colSpan="6" className="text-center py-8 text-slate-400">Belum ada data tagihan.</td></tr>
                        ) : (
                            tagihan.map((t) => (
                                <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                    <td className="px-6 py-4 font-semibold text-slate-800 dark:text-slate-100">{t.nama}</td>
                                    <td className="px-4 py-4 text-xs text-slate-600 dark:text-slate-300">{t.keterangan}</td>
                                    <td className="px-4 py-4 text-xs font-semibold text-slate-700 dark:text-slate-200">{formatRupiah(t.jumlah)}</td>
                                    <td className="px-4 py-4 text-xs text-slate-500 dark:text-slate-400">{t.jatuhTempo || '-'}</td>
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        >
                                            {t.status === 'LUNAS' ? 'Tandai Belum Lunas' : 'Tandai Lunas'}
                                        </button>
                                    </td>
                                </tr>
                            ))
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
                        <h2 className="text-lg font-bold text-slate-900 dark:text-white">Tambah Tagihan</h2>
                        <form onSubmit={handleSubmit} className="space-y-3">
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Jumlah (Rp)</label>
                                <input
                                    required
                                    type="number"
                                    min="0"
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Jatuh Tempo</label>
                                <input
                                    required
                                    type="date"
