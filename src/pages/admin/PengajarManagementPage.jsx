...
import { createPengajar, updatePengajar, setPengajarStatus, deletePengajar } from '../../services/adminServices';
...
const PengajarManagementPage = () => {
    ...
    const [editForm, setEditForm] = useState({ nama: '', email: '', mapelDiampu: '', noHp: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert(err.code === 'auth/email-already-in-use' ? 'Email tersebut sudah digunakan.' : 'Gagal menambah data pengajar: ' + err.message);
        }
    };

    const openDetail = (pengajar) => {
        setDetailPengajar(pengajar);
        setEditForm({ nama: pengajar.nama || '', email: pengajar.email || '', mapelDiampu: pengajar.mapelDiampu || '', noHp: pengajar.noHp || '' });
    };
    ...
    const handleDelete = async () => {
        if (!confirm(`Hapus data ${detailPengajar.nama} dari sistem? Akses pengajar ini ke aplikasi akan langsung dicabut.`)) return;
        setSaving(true);
        try {
                        ...
                        <form onSubmit={handleSubmit} className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-[11px] text-slate-400">
                                    Mapel diampu &amp; nomor HP bisa dilengkapi lewat Detail setelah akun dibuat.
                                </p>
                            </div>
                ...
                <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
                        <div className="flex items-center justify-between">
                            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Detail Pengajar</h2>
                            <span
                                className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                                    detailPengajar.status !== false ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'
                                }`}
                            >
                        ...
                        <form onSubmit={handleUpdate} className="space-y-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-[11px] text-amber-600">
                                    Ini hanya mengubah email kontak di sistem. Email login (Firebase Auth) tetap yang lama sampai pengajar reset password sendiri.
                                </p>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setEditForm({ ...editForm, mapelDiampu: e.target.value })}
                                    className="w-full px-3 py-2 border rounded-lg text-sm outline-none focus:border-red-500 bg-transparent"
                                />
                            </div>
                            <div>
