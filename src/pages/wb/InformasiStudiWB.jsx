...
const InformasiStudiWB = () => {
    ...
    return (
        <section className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Informasi Studi</h1>
                <p className="text-sm text-slate-500 dark:text-slate-400">Ringkasan data akademik Anda di PKBM KARTINI.</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {info.map((item) => (
                    <div key={item.label}>
                        ...
                        <p className="font-semibold text-slate-800 dark:text-slate-100 mt-0.5">{item.value}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800">
                    <h2 className="font-bold text-slate-900 dark:text-white">Mata Pelajaran {currentUser?.paket}</h2>
                </div>
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                    {loading ? (
                        <p className="px-6 py-8 text-center text-slate-400 text-sm">Memuat data mapel...</p>
                    ) : mapel.length === 0 ? (
                        <p className="px-6 py-8 text-center text-slate-400 text-sm">Belum ada mata pelajaran terdaftar untuk paket ini.</p>
                    ) : (
                        mapel.map((m) => (
                            <div key={m.id} className="px-6 py-3 text-sm font-medium text-slate-700 dark:text-slate-200">{m.nama}</div>
                        ))
                    )}
                </div>
            </div>
