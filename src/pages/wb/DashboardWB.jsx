...
const DashboardWB = () => {
  ...
  useEffect(() => {
    const stopPresensi = onSnapshot(
      query(collection(db, "presensi"), where("wbId", "==", currentUser.uid)),
      (snap) => {
        const bulanIni = currentMonthPrefix();
        setPresensiBulanIni(
          snap.docs
            .map((d) => d.data())
            .filter((p) => p.tanggal?.startsWith(bulanIni)),
        );
      },
    );
    const stopTagihan = onSnapshot(
      query(collection(db, "invoices"), where("wbId", "==", currentUser.uid)),
      (snap) => {
  ...
  const totalHadir = presensiBulanIni.filter(
    (p) => p.status === "Hadir",
  ).length;
  const kehadiranPersen = presensiBulanIni.length
    ? `${Math.round((totalHadir / presensiBulanIni.length) * 100)}%`
    : "-";

  const belumLunas = tagihan.some((t) => t.status !== "LUNAS");
  const statusTagihan =
    tagihan.length === 0 ? "-" : belumLunas ? "Belum Lunas" : "Lunas";
  const tagihanAccent =
    tagihan.length === 0
    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {cards.map((card) => (
          ...
          >
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center ${card.accent}`}
            >
