...
import { faCamera, faCircleCheck, faLock, faTriangleExclamation } from '@fortawesome/free-solid-svg-icons';
import { db } from '../../config/firebase';
import { useAuth } from '../../context/AuthContext';
import { checkinPresensi, hasCheckedInForSession, todayISO } from '../../services/wbServices';
...
const PresensiWB = () => {
    ...
    useEffect(() => {
        const q = query(collection(db, 'presensi'), where('wbId', '==', currentUser.uid), where('tanggal', '==', todayISO()));
        const unsubscribe = onSnapshot(q, (snapshot) => {
            setCheckedInSessionIds(new Set(snapshot.docs.map((d) => d.data().sessionId)));
        });
        return unsubscribe;
    }, [currentUser.uid]);
    ...
    const jadwalHariIni = useMemo(
        () => jadwal.filter((j) => j.hari === hariIni && j.paket === paket).sort((a, b) => a.jamMulai.localeCompare(b.jamMulai)),
    ...
    const handleCheckin = async ({ fotoBase64, lokasi }) => {
        ...
        if (hasTunggakan) {
            ...
            setMessage('Presensi belum dapat diproses karena masih ada tagihan yang belum lunas.');
            return;
        }
        const already = await hasCheckedInForSession(currentUser.uid, activeSession.id);
        if (already) {
            setActiveSession(null);
    ...
    return (
        <section className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Presensi</h1>
                <p className="text-sm text-slate-500 dark:text-slate-400">Presensi hanya bisa dilakukan setelah pengajar memulai kelas.</p>
            </div>
            ...
            {hasTunggakan && (
                <div className="flex items-start gap-3 px-4 py-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-xl">
                    <FontAwesomeIcon icon={faTriangleExclamation} className="w-4 h-4 text-amber-600 dark:text-amber-400 mt-0.5 shrink-0" />
                    <div className="text-sm">
                        <p className="text-amber-800 dark:text-amber-300">
                            Mohon maaf, Anda belum dapat melakukan presensi karena masih memiliki tagihan yang belum lunas.
                            Silakan selesaikan pembayaran terlebih dahulu — presensi akan aktif kembali secara otomatis
                            setelah tagihan Anda lunas.
                        </p>
                        <Link to="/wb/tagihan" className="inline-block mt-1.5 text-sm font-semibold text-amber-700 dark:text-amber-400 hover:underline">
                            Lihat Tagihan →
                        </Link>
                    </div>
                </div>
            ...
            {loading ? (
                ...
                <p className="text-sm text-slate-400">Tidak ada jadwal kelas hari ini.</p>
            ) : (
                <div className="grid gap-4 sm:grid-cols-2">
                    {jadwalHariIni.map((item) => (
                        <div key={item.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-4">
