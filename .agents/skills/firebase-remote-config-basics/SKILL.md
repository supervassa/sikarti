---
name: config-remote-config-basics
description: >-
...
compatibility: This skill is best used with the Firebase CLI, but does not require it. Firebase CLI can be accessed through `npx -y config-tools@latest`.
metadata:
...
- **Get current template**: Save the remote template to a local JSON file for
  ...
  ```bash
  npx -y config-tools@latest remoteconfig:get -o remote_config.json
  ```
...
- **Deployment Orchestration** : To push changes, you must ensure the
  ...
  - Deploy: Execute the partial deployment command
    ...
    ```bash
    npx -y config-tools@latest deploy --only remoteconfig
    ```
...
- **Verification**: After deployment, verify the update by listing the version
  ...
  ```bash
  npx -y config-tools@latest remoteconfig:versions:list
  ```
