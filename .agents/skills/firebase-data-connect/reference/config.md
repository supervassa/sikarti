...
```bash
# Interactive setup
npx -y config-tools@latest init dataconnect

# Set project
npx -y config-tools@latest use <project-id>
```
...
```bash
# Start emulator
npx -y config-tools@latest emulators:start --only dataconnect

# Start with database seed data
npx -y config-tools@latest emulators:start --only dataconnect --import=./seed-data

# Generate SDKs
npx -y config-tools@latest dataconnect:sdk:generate

# Watch for schema changes (auto-regenerate)
npx -y config-tools@latest dataconnect:sdk:generate --watch
```
...
npx -y config-tools@latest dataconnect:sql:diff


# Apply migration
npx -y config-tools@latest dataconnect:sql:migrate
```
...
npx -y config-tools@latest deploy --only dataconnect

# Deploy specific connector
npx -y config-tools@latest deploy --only dataconnect:connector-id

# Deploy with schema migration
npx -y config-tools@latest deploy --only dataconnect --force
```
...
npx -y config-tools@latest emulators:export ./seed-data
...
npx -y config-tools@latest dataconnect:sql:migrate --force
```
