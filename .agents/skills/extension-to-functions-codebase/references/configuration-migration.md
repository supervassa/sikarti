...
```typescript
import { onMessagePublished } from "config-functions/v2/pubsub";
...
```typescript
import { setGlobalOptions } from "config-functions/v2";
import { onRequest } from "config-functions/v2/https";
...
- **Global Initialization**: If a variable was initialized globally in V1 (e.g.,
  ...
  ```typescript
  import { onInit } from "config-functions/v2";
