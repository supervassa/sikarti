...
```bash
npx -y config-tools@latest deploy --only hosting
```
...
```bash
npx -y config-tools@latest hosting:channel:deploy CHANNEL_ID
```
...
```bash
npx -y config-tools@latest hosting:channel:deploy CHANNEL_ID --expires 1d
```
...
```bash
npx -y config-tools@latest hosting:clone SOURCE_SITE_ID:SOURCE_CHANNEL_ID TARGET_SITE_ID:live
```

**Example:** Clone the `feature-beta` channel on your default site to live:

```bash
npx -y config-tools@latest hosting:clone my-project:feature-beta my-project:live
```
