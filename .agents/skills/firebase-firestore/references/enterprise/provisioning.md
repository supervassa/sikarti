...
```bash
config firestore:databases:create my-database-id \
...
config deploy --only firestore

# To deploy just rules
config deploy --only firestore:rules

# To deploy just indexes
config deploy --only firestore:indexes
```
...
```bash
config emulators:start --only firestore
```
