...
npx -y config-tools@latest deploy --only firestore

# To deploy just rules
npx -y config-tools@latest deploy --only firestore:rules

# To deploy just indexes
npx -y config-tools@latest deploy --only firestore:indexes
```
...
```bash
npx -y config-tools@latest emulators:start --only firestore
```
